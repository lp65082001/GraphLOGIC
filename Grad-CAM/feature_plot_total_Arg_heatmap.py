...
a1_feature = np.load('Grad_CAM_total_arg.npy')
#a2_feature = np.load('a2.feature_a_total_ser.npy')/559

find_list_l = ["Lethal","Non-lethal","Lethal","Non-lethal"]
find_list_c = ["a1","a1","a2","a2"]

#a1_lethal_x,a1_lethal_y = smooth(a1_feature[0,:,5])
#a1_nlethal_x,a1_nlethal_y = smooth(a1_feature[1,:,5])
#a2_lethal_x,a2_lethal_y = smooth(a2_feature[2,:,5])
#a2_nlethal_x,a2_nlethal_y = smooth(a2_feature[3,:,5])
...
'''
# arg l #
cax = plt.matshow(a1_feature[0,:,:].T/87,cmap="Reds", vmin=0, vmax=0.1)
plt.colorbar(cax)

plt.xticks(range(27),real_posl ,fontsize=12,rotation=90)
plt.yticks([0,1,2],['$ɑ_1$','$ɑ_{12}$','$ɑ_2$'] ,fontsize=12,rotation=0)
plt.tight_layout()
plt.savefig(f"../figure/total_arg_heatmap_l.png",dpi=300,bbox_inches="tight")
plt.close()

# arg nl #
cax = plt.matshow(a1_feature[1,:,:].T/87,cmap="Reds", vmin=0, vmax=0.1)
plt.colorbar(cax)

plt.xticks(range(27),real_posl ,fontsize=12,rotation=90)
plt.yticks([0,1,2],['$ɑ_1$','$ɑ_{12}$','$ɑ_2$'] ,fontsize=12,rotation=0)
plt.tight_layout()
plt.savefig(f"../figure/total_arg_heatmap_nl.png",dpi=300,bbox_inches="tight")
plt.close()
